...
public class HyperbolicCoral extends PApplet {
	...
	float strength = 0.1f;  // Strength of springs between nodes
	float damping = 0.05f;  // Damping of springs between nodes
	...
	float drag = 0.03f;  // Physics System drag (friction)
	...
	void makeCoral(){
		...
		float ringmult = (patterncount*1.0f)/pattern.length;
		...
		start = physics.makeParticle(1.0f,0,0,0);
		start.makeFixed();
		...
		for (int j=0; j<numgenerations; j++){
			...
			for (int i=0; i<numnodes; i++){
				...
				Particle P  = physics.makeParticle(1.0f,cos(newangle)*newradius,sin(newangle)*newradius,random(-5,5));
				ring.add(P);
